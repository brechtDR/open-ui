...
- The control is **not** hidden and is **not** rendered in its disabled state. `disabled` remains an author-controlled attribute; an empty group is simply a group that has nothing to drag yet.
- There are no thumbs, so there are no tab stops inside the group, and pointer interaction on the track does nothing. There is no nearest thumb to move and no segment to translate, so `interaction` has no observable effect.
- The track is one uninterrupted region. `::slider-segment(1)` addresses it; `::slider-fill` does not apply, since there is no thumb to fill up to.
...
The alternative considered was to have an empty group imply a single default thumb, the way `<input type="range">` with no `min`/`max` gives you one thumb between 0 and 100. That was rejected: an implied thumb has no backing `<input type="range">`, so the author loses the element, its IDL, its `name`, and its value in form submission, which makes it meaningfully different from a group of one rather than a shorthand for it.
...
Until that change lands, a `<legend>` inside `<rangegroup>` is invalid markup and, in a non-supporting browser, is not associated with anything. Authors who need markup that validates today can wrap the group in a `<fieldset>` and put the `<legend>` there instead; the group is then named through `aria-labelledby` on the `<rangegroup>`. The examples in this explainer use the proposed form. The Lit proof-of-concept keeps the same authoring pattern (`<legend slot="legend">`) but copies the legend's text onto the group's `aria-label`: `aria-labelledby` cannot resolve an IDREF from shadow DOM to light DOM, so the ID-based approach leaves the group unnamed. That is a polyfill limitation, not part of the proposal.
...
In range-oriented mode (`interaction="range"`), the proof-of-concept also implements a keyboard counterpart to segment drag: **Shift+Arrow** (and **Shift+Page Up/Down** for a larger step) on a focused thumb translates the adjacent segment, preserving its width. The preferred pair is the one in the travel direction; if that pair is missing or disabled, the other pair that includes the thumb is used. Without Shift, arrows still move a single thumb, so every end state remains reachable. Whether this gesture is the right contract — versus making the segment itself a tab stop — remains open on [#1511](https://github.com/openui/open-ui/issues/1511).
...
<rangegroup name="budget" min="0" max="1000000" list="budget-stops">
    ...
    <label>Budget cap <input type="range" name="budget-cap" value="10000" /></label>
</rangegroup>
...
- `interaction`: **Working name — needs bikeshed** ([#1460](https://github.com/openui/open-ui/issues/1460)). Not a committed HTML attribute; do not use `draggable`. Keywords: `endpoints` (default), where a track press moves the nearest thumb, and `range`, where a track press moves a segment and never a single thumb. See [Track Click Behavior](#track-click-behavior). Other names that have been floated: `selects`, `pointermode`, `rangemode`, `adjust`. Watch collisions with `inputmode` and with the CSS [`interactivity`](https://drafts.csswg.org/css-ui-4/#interactivity-property) property, which controls inertness and is a near-miss for this name.
...
In range-oriented mode, **clicking the track never moves a thumb**. Every press that does not land on a thumb moves a *segment*, and only a direct drag of a thumb changes an endpoint:

- Pressing **inside** a segment starts dragging that segment. Both bounding thumbs move together and the segment keeps its width.
- Pressing **outside** every segment (before the first thumb or after the last) moves the **nearest** segment to the pointer, and dragging continues from there. This is the part that changed: the earlier draft kept nearest-thumb behavior in those outer regions, which made the control feel like it had two unrelated interaction models depending on where you pressed.
...
A group with fewer than two thumbs has no segment, so `interaction="range"` has nothing to translate. Such a group keeps endpoint-oriented behavior: a track press moves the single thumb, and an empty group does nothing at all.
...
The Lit [proof-of-concept](https://brechtdr.github.io/enhanced-range-slider-poc/) implements these keywords for **any** thumb count. With three or more thumbs the pointer position picks the pair: pressing inside a segment names the two thumbs that bound it, and pressing outside all of them names the closest pair, so there is no need to decide up front which interval is "the" interval. A dragged segment is clamped by the thumbs on either side of it, by any per-thumb `min`/`max`, and by `stepbetween`, so it stops rather than squashing. Segments whose bounding thumbs are not both enabled are not draggable and are skipped when looking for the nearest one. A library vs in-app survey is on [#1460](https://github.com/openui/open-ui/issues/1460): libraries default to nearest-thumb and offer interval-drag as opt-in, which is evidence for `endpoints` as the default if a mode ships, but the focus-group comparison is what should decide it.
...
- **Keyboard and focus for a draggable segment** ([#1511](https://github.com/openui/open-ui/issues/1511), split from [#1460](https://github.com/openui/open-ui/issues/1460)): If range-oriented dragging ships, how does a keyboard or AT user translate a segment without a pointer? The proof-of-concept currently offers **Shift+Arrow** on a focused thumb (see [Keyboard Interaction](#keyboard-interaction)), which keeps the segment out of the tab order. Alternatives still on the table: make the segment itself a tab stop / AT object, or decide that per-thumb arrow keys are enough because every end state is reachable that way. The gesture vs tab-stop choice, and how (or whether) the interaction mode is exposed in the accessible description, need a real resolution before a mode ships.
