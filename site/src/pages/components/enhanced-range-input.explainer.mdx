...
The diagrams below show how the named parts sit on a single-thumb range and on a two-thumb `<rangegroup>`, and how a **segment** (paint region) differs from an **interval** (draggable region).
...
Based on community feedback and discussions, we propose a new `<rangegroup>` element rather than a new attribute or a second range input type. It is a wrapper that contains any number of `<input type="range">` elements and paints them as a single slider on a shared track.
...
In a supporting browser this is enough: omitted child `min`/`max` resolve to the group's values. Repeat them on the children only when a non-supporting browser must keep the same domain; otherwise those fallbacks default to 0–100 and clamp `-25` to `0`. See [Progressive Enhancement](#progressive-enhancement) and [Non-linear value scales](#non-linear-value-scales) for the duplicated spelling.
...
A group of **zero** is also valid. A `<rangegroup>` that contains no `<input type="range">` renders **an empty track**: the track is painted as normal, with no thumbs on it ([#1515](https://github.com/openui/open-ui/issues/1515), [telecon 10 Sep 2026](https://www.w3.org/2026/09/10-openui-minutes.html)).
...
- The control is **not** hidden and is **not** rendered in its disabled state. `disabled` remains an author-controlled attribute; an empty group is simply a group that has nothing to drag yet. It is not exposed as `aria-disabled` either.
- There are no thumbs, so there are no tab stops inside the group, and pointer interaction on the track does nothing. There is no nearest thumb to move and no interval to translate, so `interaction` has no observable effect.
- The track is one uninterrupted region. `::slider-segment(1)` addresses it; `::slider-fill` does not apply, since there is no thumb to fill up to. If the group has `list`, tick marks still render: they belong to the track, not to any thumb.
- Nothing is submitted, because submission comes from the child inputs. This is the same rule as a `<rangegroup>` whose children are all disabled, though the reason differs: there are no entries rather than excluded entries.
- The group remains in the accessibility tree. Like a `<fieldset>` with no controls, it is exposed as a `group` with its accessible name from `<legend>` or `aria-labelledby`, and is not pruned — so a script-filled group does not flicker in and out of assistive technology.
- It is **not** exposed as a slider: no `slider` role and no `aria-valuenow` / `aria-valuemin` / `aria-valuemax` on the group itself. Those belong to the child inputs. The painted track is presentational.
- With no focusable descendants it is reachable with a browse or virtual cursor but is not a Tab stop, and the group does not become focusable to compensate.
- The HTML `name` attribute is not an accessible name; it identifies the group in `form.elements` (see [Form Association and Naming](#form-association-and-naming)). An empty group with `name` but no legend has no accessible name and most assistive technology will not announce it, so a group that matters before it has thumbs needs a legend.

The empty track also gives scripted groups a coherent starting point. Because thumbs are backed by real light-DOM inputs, appending the first `<input type="range">` turns an empty group into a single-thumb group, appending a second turns it into a two-thumb group, and removing them walks back down the same path, ending at the empty track rather than at a control that suddenly grows a thumb of its own.
...
<form>
    <rangegroup name="price-range" min="0" max="1000">
        ...
        <label>Minimum Price <input type="range" name="price-min" min="0" max="1000" value="100" /></label>
        <label>Maximum Price <input type="range" name="price-max" min="0" max="1000" value="750" /></label>
    </rangegroup>
</form>
```
...
In range-oriented mode (`interaction="range"`), the proof-of-concept also implements a keyboard counterpart to interval drag: **Shift+Arrow** (and **Shift+Page Up/Down** for a larger step) on a focused thumb translates the adjacent interval, preserving its width. The preferred pair is the one in the travel direction; if that pair is missing or disabled, the other pair that includes the thumb is used. Without Shift, arrows still move a single thumb, so every end state remains reachable. When the focused thumb has no adjacent interval (fewer than two enabled thumbs), Shift+Arrow does nothing; plain arrows still step. Whether this gesture is the right contract — versus making the interval itself a tab stop — remains open on [#1511](https://github.com/openui/open-ui/issues/1511) (that issue still uses the older "segment" wording for the draggable region).

### Screen Reader Announcements

When a user navigates into a `<rangegroup>` for the first time (focusing the first thumb), the group's overall `min` and `max` should be announced, plus `stepbetween` when it applies (two or more thumbs). Each individual thumb then announces its own effective `min`, `max`, and current value — that is, the per-thumb clamps from the child input's `min`/`max` attributes, not only the group's. The group's name comes from its legend (or `aria-labelledby`); it should not be re-prefixed onto every thumb label, or every arrow-key stop re-announces the group. This layered announcement model keeps group context and per-handle state distinct.
...
Whether the group should instead inherit `list`, `min`, and `max` down to its children automatically, so the duplication is unnecessary, is an open question. If the group's values act as defaults for children that omit them, duplication is needed only while unsupporting browsers matter. The constraint on any inheritance rule is that both spellings must produce the same result in a supporting browser.

### Non-linear value scales

A recurring request is a range whose value does not grow evenly along the track ([#1464](https://github.com/openui/open-ui/issues/1464)). A price filter from 1 to 1,000,000 is the classic case: on a linear scale everything below 500,000 sits in the left half, and a one-pixel drag near the middle jumps by thousands, so the values most people actually want are unreachable. The same shape of problem turns up in audio frequency and gain, zoom levels, file sizes, and anything spanning several orders of magnitude.

The [10 September 2026 telecon](https://www.w3.org/2026/09/10-openui-minutes.html) resolved to first build this out of what a range input can already express, namely a `<datalist>` with unevenly spaced options, and to document the result here rather than to introduce a scale attribute now:

```html
<rangegroup name="budget" min="0" max="1000000" list="budget-stops">
    <legend>Budget cap</legend>
    <label>Budget cap
        <input type="range" name="budget-cap" min="0" max="1000000" value="10000" list="budget-stops" />
    </label>
</rangegroup>
...
The child repeats `min`, `max`, and `list` so a non-supporting browser preserves the domain and the ticks. Without them the fallback would be a default 0–100 slider and `value="10000"` would clamp to 100, which contradicts the progressive-enhancement story. In a supporting browser the duplication is inert: identical child `min`/`max` intersect with the group's to the same range, and the group's `list` already takes precedence. The duplication is therefore a fallback technique, which is exactly why it can eventually be dropped once inheritance (see above) lands or unsupporting browsers no longer matter.
**The result of building it: this gives you a working keyboard log scale and a broken pointer one.** Keyboard stepping moves one option at a time, so a single arrow press is one notch whether it is worth €900 or €900,000, which is exactly the behavior a logarithmic control should have. The pointer is a different story, and it does not survive contact with a real scale. Because the track coordinate space is linear and snapping picks the nearest option *by value*, the six stops above are not reachable in any practical sense: everything below €100k occupies the leftmost tenth of the track, and more than half the track resolves to €1M. In the [proof-of-concept](https://brechtdr.github.io/enhanced-range-slider-poc/#non-linear-scale) a pointer sweep across the whole track reaches three of the six stops, while the arrow keys reach all six. €10,000 puts the thumb at 1% of the track; on an evenly spaced version of the same scale it sits at 60%.

That asymmetry is the useful finding, because it locates the problem precisely. Stepping already works in *stop-index* space, which is the space a non-linear scale needs. Everything that is broken is broken because positioning and hit-testing still happen in *value* space. Whatever the eventual solution is, that is the thing it has to change, and it is not something an author can patch from the outside.

So the datalist route is a floor rather than an answer. It is genuinely useful when the stops are only mildly uneven, and it needs no new platform surface; it stops being useful at exactly the point where authors reach for a log scale in the first place. The gaps below are each worth their own issue rather than being settled here:

- **Stop spacing.** Options are placed proportionally to their value, so in the example above four of the six stops crowd into the leftmost tenth of the track. A logarithmic control wants the opposite: roughly equal track distance between consecutive stops. Whether spacing should be value-proportional, evenly distributed, or author-selectable is the central unanswered question, and per the finding above it is also the one that would make the pointer usable.
- **Snapping versus free movement.** A `<datalist>` on `<input type="range">` today is a suggestion rather than a constraint: pointer input snaps, but keyboard input in Firefox ignores the options entirely. Authors asking for a log scale usually want one of two clearly different things: only the listed values, or a continuous scale that merely marks them. That looks like an explicit opt-in, for example a `step` keyword meaning "follow the list" alongside the numeric form, rather than something inferred from the presence of `list`.
- **Step semantics on a continuous non-linear scale.** If movement between stops is continuous, a fixed `step` is wrong: the natural definition is multiplicative, where one step scales the current value by a factor, so pressing an arrow key covers a consistent visual distance anywhere on the track. That interacts with per-thumb `step` and with `stepbetween`, which is expressed as an absolute distance.
- **Zero on a logarithmic scale.** A logarithmic mapping is undefined at zero, yet `min="0"` is precisely what a price filter wants. The datalist form tolerates `€0` only because it never computes a log; any future `scale` keyword has to say what happens there (clamp to a positive epsilon, reject zero, or use a piecewise mapping).
- **A declarative scale, or a scripted one.** The original issue floats `scale="logarithmic"` and, as an escape hatch, a bidirectional `valueToPosition` / `positionToValue` pair for arbitrary mappings. Both are deferred. Neither is ruled out, but the datalist route has to be shown insufficient first, and the scope has to stay small enough not to become a second multi-year project alongside `<rangegroup>` itself.

Nothing here blocks authors in the meantime: values can still be transformed in script on `input` / `change`, which remains more expressive than any attribute. The cost of that workaround is visible in the proof-of-concept's second slider, where even spacing is bought by making the slider hold an exponent instead of a price, so the real value never reaches the input and is absent from `value`, from form submission, and from validation. Whatever lands should also degrade the same way the rest of this proposal does, with an unsupporting browser falling back to a linear native range.
...
This explainer uses two related terms that must not be conflated (see [Anatomy](#anatomy)):

- A **segment** is a paint region addressed by `::slider-segment(n)`. Segments are numbered by position along the track, with 1 at the start (based on writing mode).
- An **interval** is the draggable region bounded by a thumb on each side. Interval drag is what `interaction="range"` translates; see [Track Click Behavior](#track-click-behavior).

With *N* thumbs there are *N*+1 segments and `max(N−1, 0)` intervals. The interval between thumb *i* and thumb *i*+1 coincides with `::slider-segment(i+1)`. The first and last segment are never intervals: translating either would need a thumb at the track edge.

```
|--- segment(1) ---o--- segment(2) ---o--- segment(3) ---|
track start     thumb A           thumb B        track end
                 \____ interval ____/
```
...
The number of segments is determined by the number of thumbs. With zero thumbs there is one segment (the whole track). With one thumb there are two segments (track start to thumb, and thumb to track end), matching the `::slider-fill-under` / `::slider-fill-over` split [resolved](https://github.com/w3c/csswg-drafts/issues/12419) for a single range. With two thumbs there are three segments (before the first handle, between handles, and after the last handle), and so on.
...
- `interaction`: **Working name — needs bikeshed** ([#1460](https://github.com/openui/open-ui/issues/1460)). Not a committed HTML attribute; do not use `draggable`. Keywords: `endpoints` (default), where a track press moves the nearest thumb, and `range`, where a track press moves an interval and never a single thumb. See [Track Click Behavior](#track-click-behavior). Other names that have been floated: `selects`, `pointermode`, `rangemode`, `adjust`. Watch collisions with `inputmode` and with the CSS [`interactivity`](https://drafts.csswg.org/css-ui-4/#interactivity-property) property, which controls inertness and is a near-miss for this name.
...
When a child omits `min` or `max`, it takes the group's value rather than the `<input type="range">` defaults of 0 and 100. Without that rule a group with `min="0" max="1000"` and a child that only sets `value="750"` would clamp the thumb to 100 in a supporting browser — the opposite of what authors expect. Authors who need the same domain in a non-supporting browser should still repeat `min` and `max` on the child (see [Datalist Integration](#datalist-integration) and [Non-linear value scales](#non-linear-value-scales)).
...
This is the **endpoint-oriented** default (`interaction="endpoints"`). A **range-oriented** keyword (`interaction="range"`) is proposed as a working API for calendar blocks, alarm windows, and video in/out, where the width of the interval matters more than either of its ends.

In range-oriented mode, **clicking the track never moves a thumb**. Every press that does not land on a thumb moves an *interval*, and only a direct drag of a thumb changes an endpoint. (An interval is the region bounded by two thumbs; see [Multi-Color Range Segments](#multi-color-range-segments) for how that differs from a paint segment.)

- Pressing **inside** an interval starts dragging that interval. Both bounding thumbs move together and the interval keeps its width.
- Pressing **outside** every interval (in the first or last segment — before the first thumb or after the last) moves the **nearest** interval to the pointer, and dragging continues from there. This is the part that changed: the earlier draft kept nearest-thumb behavior in those outer regions, which made the control feel like it had two unrelated interaction models depending on where you pressed.
- Dragging a thumb directly still adjusts that one endpoint, exactly as in endpoint-oriented mode.

This was [resolved](https://www.w3.org/2026/09/10-openui-minutes.html) on 10 September 2026 ([#1460](https://github.com/openui/open-ui/issues/1460)). The point of removing the nearest-thumb fallback entirely is to make the two keywords **two cleanly distinct experiences** rather than two variations that share most of their behavior, so they can be put in front of users as an either/or choice. Which of them should be the default is a question for that testing, not something this explainer settles.

An interval requires two **enabled** thumbs. With fewer (one thumb, one enabled thumb, or none), `interaction="range"` has nothing to translate and a track press does nothing. Direct thumb drag and keyboard stepping still work, so no value becomes unreachable. Falling back to endpoint-oriented track-click here would reintroduce the hybrid the resolution removed, and would make identical markup behave differently depending on how many thumbs happen to be enabled at that moment. Authors who want track-click on a single-thumb control want `endpoints`.

The attribute name and its keywords are a **working name and need bikeshed** on [#1460](https://github.com/openui/open-ui/issues/1460) (not a second issue). `interaction` is close to the CSS [`interactivity`](https://drafts.csswg.org/css-ui-4/#interactivity-property) property and to `inputmode`, so a different name may well win. `draggable` is rejected because it collides with the existing global attribute. Nothing in this section depends on the final spelling: the two behaviors are what matter. Related: [#1278](https://github.com/openui/open-ui/issues/1278). Keyboard and focus for a draggable interval: [#1511](https://github.com/openui/open-ui/issues/1511) (that issue still uses the older "segment" wording).

The Lit [proof-of-concept](https://brechtdr.github.io/enhanced-range-slider-poc/) implements these keywords for **any** thumb count. With three or more thumbs the pointer position picks the pair: pressing inside an interval names the two thumbs that bound it, and pressing outside all of them names the closest pair, so there is no need to decide up front which interval is "the" interval. A dragged interval is clamped by the thumbs on either side of it, by any per-thumb `min`/`max`, and by `stepbetween`, so it stops rather than squashing. Intervals whose bounding thumbs are not both enabled are not draggable and are skipped when looking for the nearest one. (The PoC currently falls back to endpoint behavior when fewer than two thumbs are enabled; that should be updated to match the inert rule above.) A library vs in-app survey is on [#1460](https://github.com/openui/open-ui/issues/1460): libraries default to nearest-thumb and offer interval-drag as opt-in, which is evidence for `endpoints` as the default if a mode ships, but the focus-group comparison is what should decide it.
...
- **Handle count** ([#1337](https://github.com/openui/open-ui/issues/1337), [#1183](https://github.com/openui/open-ui/issues/1183)): A `<rangegroup>` may contain **any number** of range inputs. A group of one still renders the composite slider; a group of none renders an empty track. There is no upper limit; authors choose how many thumbs their use case needs. Whether 3+ thumb patterns are common enough to also ship a simpler two-thumb-only API is still community feedback, not a cap on `<rangegroup>`.
- **Empty group** ([#1515](https://github.com/openui/open-ui/issues/1515), [telecon 10 Sep 2026](https://www.w3.org/2026/09/10-openui-minutes.html)): **Resolved:** a `<rangegroup>` with no `<input type="range">` has an empty track. It is not hidden and not disabled, and it does not imply a default thumb, because an implied thumb would have no backing input, IDL, or submitted value. See [Empty `<rangegroup>`](#empty-rangegroup).
...
- **Non-linear value scales** ([#1464](https://github.com/openui/open-ui/issues/1464), [telecon 10 Sep 2026](https://www.w3.org/2026/09/10-openui-minutes.html)): **Resolved:** prototype the non-linear scale with `<datalist>`, as far as `<input type="range">` already allows, and document that in this explainer. No `scale` attribute and no scripted mapping in v1. The shortcomings that surfaced while discussing it (stop spacing, snapping versus free movement, step semantics on a continuous scale) are tracked as separate issues. See [Non-linear value scales](#non-linear-value-scales).
- **Behavior of the range-oriented pointer mode** ([#1460](https://github.com/openui/open-ui/issues/1460), [telecon 10 Sep 2026](https://www.w3.org/2026/09/10-openui-minutes.html)): **Resolved:** in range-oriented mode, interval dragging is the whole model. Click-to-move-a-thumb is removed there, including in the regions outside the outermost thumbs, where a press moves the nearest interval instead. When there is no interval (fewer than two enabled thumbs), a track press does nothing — the mode does not fall back to endpoint behavior. This keeps the two modes as genuinely different experiences so they can be compared with user testing. The attribute name, its keywords, and which mode is the default are all still open (see [Considerations and Open Questions](#considerations-and-open-questions)).
...
- **Segment numbering when thumbs overlap or cross**: Segment numbering is positional (see [Multi-Color Range Segments](#multi-color-range-segments)) while thumb overlap is allowed ([#1184](https://github.com/openui/open-ui/issues/1184)), so dragging one thumb past another renumbers the segments and the colors change under the user's hand. Numbering by child DOM order instead would keep each color tied to an input but let painted colors reorder along the track. Related: when two thumbs coincide, does the zero-width segment still generate a box? And should `::slider-segment()` accept `An+B` / `even` / `odd`, since a script-driven group (the empty-state scenario) cannot enumerate indices up front?
- **Which pointer model should be the default** ([#1278](https://github.com/openui/open-ui/issues/1278), [#1460](https://github.com/openui/open-ui/issues/1460)): The two behaviors are now settled and both implemented (see [Track Click Behavior](#track-click-behavior)); what is not settled is which one a `<rangegroup>` gets when the author says nothing, or whether a mode keyword ships at all. The explainer currently writes `endpoints` as the default, supported by a survey comment on #1460 showing that libraries default to nearest-thumb and offer interval-drag as opt-in. That is not a formal resolution. The 10 September 2026 telecon asked for the two modes to be built as deliberately different experiences so the question can be answered with user feedback (even something as light as a public poll) rather than room preference. **The name and keywords also still need bikeshed** — keep that on #1460 rather than a new issue; `draggable` is rejected. What stays unresolved separately is reaching an interval **without** a pointer: [#1511](https://github.com/openui/open-ui/issues/1511).
- **Keyboard and focus for a draggable interval** ([#1511](https://github.com/openui/open-ui/issues/1511), split from [#1460](https://github.com/openui/open-ui/issues/1460)): If range-oriented dragging ships, how does a keyboard or AT user translate an interval without a pointer? The proof-of-concept currently offers **Shift+Arrow** on a focused thumb (see [Keyboard Interaction](#keyboard-interaction)), which keeps the interval out of the tab order. Alternatives still on the table: make the interval itself a tab stop / AT object, or decide that per-thumb arrow keys are enough because every end state is reachable that way. The gesture vs tab-stop choice, and how (or whether) the interaction mode is exposed in the accessible description, need a real resolution before a mode ships. (The issue title still says "segment"; this explainer now reserves that word for the CSS paint region.)
- **Dynamic `aria-valuemin` / `aria-valuemax` under neighbor constraints**: Per-thumb `min`/`max` clamps are reflected in each thumb's announced bounds (see [Screen Reader Announcements](#screen-reader-announcements)). Should `stepbetween` and neighboring thumbs also rewrite those bounds as the user moves — so a thumb that cannot currently go below 40 because of its neighbor announces `min 40` — or should announced bounds stay at the authored clamps and leave collision feedback to the interaction itself? Dynamic bounds are more accurate; static authored clamps are simpler and match how native range inputs behave today.
...
- **How far non-linear scales should go** ([#1464](https://github.com/openui/open-ui/issues/1464)): The datalist form described in [Non-linear value scales](#non-linear-value-scales) is what v1 offers, and prototyping it showed it only half works: stepping is fine, pointer positioning is not, because the track stays in value space. The open parts are whether option spacing should stay value-proportional or become evenly distributed (or author-selectable), which is the one that would fix the pointer; whether a list should be able to constrain values rather than merely suggest them; what `step` means on a continuous non-linear scale; and whether a declarative `scale` keyword or a scripted position mapping is eventually needed. Each of those is being split into its own issue so this one does not become a second project the size of `<rangegroup>`.
